...
public class QueryVideo {
    ...
    private static List<MovieVideo> extractFeatureFromJson(String jsonResponse) {
...
//            Iterate through the response to fill all the items with details
            for (int i = 0; i < jsonArrayResults.length(); i++) {
                ...
                JSONObject currentVideo = jsonArrayResults.getJSONObject(i);
                String videoIdentifier;
                String videoType;
                videoIdentifier = currentVideo.optString("key");
                videoType = currentVideo.optString("type");
                MovieVideo movieVideoItem = new MovieVideo(videoIdentifier, videoType, movieId);
                movieVideos.add(movieVideoItem);
            }
        } catch (JSONException e) {
            Log.e("Query Utils", "Problem parsing JSON Response", e);
        }
        Log.v("MovieReviews", movieVideos.toString());
        return movieVideos;
...
//    Open an HTTP connection and try to make the request successfully
    ...
    public static List<MovieVideo> fetchMovieVideos(URL url) {
